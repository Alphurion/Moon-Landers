...
/*
Returns 'a' if airborne, 'c' if not airborne and velocity <= -2, and 'T' if not airborne and
velocity is within reason > -2
*/
char lander::get_status() const
{
  ...
  else if ((this->altitude == 0.0) && (this->velocity <= -2.0))
  {
    return 'c'; // Crashed.
  }
  else
...
void lander::simulate()
{
  ...
  */
  ...
  this->velocity += v;
  this->altitude += TIME * this->velocity;
  switch ((*this).get_status())
  {
  case 'c': // Crashed
    this->stillLanding = false;
    break;
  case 'a': // Airborne
    this->stillLanding = true;
    break;
  case 'T': // Landed
    this->stillLanding = false;
    break;
  ...
  if (this->flow_rate > 0)
  {
    this->fuel_amount -= (TIME * (max_fuel_consumption_rate * abs(v)));
  }
...
bool lander::operator<(const lander &rhs) const
{
  ...
  {
    return this->lander_mass > rhs.lander_mass;
  }
